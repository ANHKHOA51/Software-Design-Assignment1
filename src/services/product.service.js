...
function searchPageByKeywords(keywords, limit, offset, userId, logic = 'or', sort = '') {
...
function countByKeywords(keywords, logic = 'or') {
  // Remove accents from keywords for search
  const searchQuery = removeAccents(keywords);
...
function findByCategoryIds(categoryIds, limit, offset, sort, currentUserId) {
  ...
  (async () => {
    try {
      ...
      if (seller && seller.email) {
        emailPromises.push(mailService.notifyNewBid(productUrl, seller, result, currentBidder));
      }

      if (currentBidder && currentBidder.email) {
        const isWinning = result.newHighestBidderId === result.userId;
        emailPromises.push(mailService.notifyHighestBid(productUrl, currentBidder, isWinning, result));
      }
      ...
      if (previousBidder && previousBidder.email && result.priceChanged) {
        ...
        emailPromises.push(mailService.notifyOutBid(productUrl, previousBidder, wasOutbid, result));
      }
...
const sendCommentNotificationEmails = async ({ productId, content, parentId, userId, req }) => {
  const product = await findByProductId2(productId, null);
  ...
  if (isSellerReplying && parentId) {
    ...
    for (const [recipientId, recipient] of recipientsMap) {
      try {
        await mailService.notifyQuestionAnswered(productUrl, recipient, seller, product, content);
      } catch (emailError) {
    ...
    if (parentId) {
      await mailService.notifyNewReply(productUrl, seller, product, commenter, content);
    } else {
      await mailService.notifyNewQuestion(productUrl, seller, product, commenter, content);
    }
  }
};
...
export const getProductByCategories = async (req, res) => {
  ...
  const list = await findByCategoryIds(categoryIds, limit, offset, sort, userId);
...
export const getProductBySearch = async (req, res) => {
  ...
  const list = await searchPageByKeywords(keywords, limit, offset, userId, logic, sort);
  const products = await prepareProductList(list);
  const total = await countByKeywords(keywords, logic);
  const { totalCount, nPages, from, to } = getPaginationSummary(total.count, page);
...
export const getProductDetail = async (req, res) => {
  ...
  const product = await findByProductId2(productId, userId);
...
export const unrejectBidder = async (req, res) => {
  ...
  try {
    ...
    const product = await findByProductId2(productId, sellerId);
